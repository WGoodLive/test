...
其实就是给上章内容实现页表分配，使用恒等映射就行，然后和上章代码没啥区别，就陷入的跳转，统一交给跳板处理，需要人为修改的代码不多，注意跳板是更改stap用的，我看答案(urore)应该是：尽管共用页表，但是satp还是不一样的，类似于每个应用的satp的虚拟地址之间是等距的！实现非恒等映射的话，内核是一定不能用随机映射的，不能保证指令的平滑进行，可以把内核放在偏低的地址恒等映射，然后每个应用映射跟上一章差不多。

==上面的理解有问题==

**额**，看到后面评论，感觉他是想让我们实现操作系统的另一种内核页面分配方式：是说每个进程一个页表，低地址空间为应用的数据，高地址空间为内核的数据。所有进程共享高地址空间的内核数据，但低地址空间的应用数据每个应用是不同的。**每个应用都有一个自己的内核页，这样陷入不用改satp,但是任务切换是需要切换satp的**

### 按需分配

sbrk的时候，我那些手续都是正常走的，这样保证：不会被别的应用在你所有的时候使用它，导致你想使用的时候无法使用。只不过，分配的时候，我会在触发pagefault的时候，再进行分配页面！

### cow机制（想完成）

扩展内核，支持基于缺页异常的COW机制。（初始时，两个任务共享一个只读物理页。当一个任务执行写操作后，两个任务拥有各自的可写物理页）

我的想法，既然FrameTracker储存着页面的生命周期，能不能让一个FrameTracker储存在两个页表里，这样某个FrameTracker死了，由于另一个没死，生命周期还在，也不用加计数器？**不行**，因为Rust所有权机制，如果要使用这种方法，可以考虑用高级的堆结构，或者自己实现一个！

我还思考了一下，感觉不能用RC/Arc，这样需要改数据结构，对于这种相对于全部的页面分配而言，少之又少的操作，把原来的数据结构改了，得不偿失，打算自己写一份类RC。
...
> 在实现cow机制过程中我的误区：
>
> 1. 我认为vpn在from_pte_create的时候，会得到一个物理地址，就是想给他加的物理地址，所以另个应用的vpn如果搜索到了相同的物理地址，由于这个物理地址已经被分配是有效的，所以他无法再map,这个想法错误的！他vpn只要是第一次匹配，得到的刚开始都是0x0，所以一定不合法，一定可以map，然后绑定没有生命周期的ppn!
>
> 2. 我认为frametracker与ppn是一一对应的，实际不然，由于ppn可以Copy，所以可以给一个ppn申请多个frametracker，然后，再释放frametracker的时候，他会调用自己的drop函数，怎么处理看我们自己！
>
>    **综上**，我们没有必要去用Rc什么的
...
1. 在堆上首先实现共享物理页，
2. 实现系统调用，
3. 改变页面访问属性，然后增加访问异常处理函数
4. 实现页面复制与重新处理函数

实现难度

- 堆分配(前期认为不能使用sbrk，因为他的map的物理页面分配没有办法指定某个物理页)
- 系统调用，如何知道跟谁建立连接
- 如何通过系统调用返回某个值？？？想实现了
- 新的数据结构储存页面的使用次数

1. 我认为data_frame就是存生命周期的，查找用的是pte

新的实现方式：

1. 主动方先使用sbrk新建一个堆页，然后获取物理页号ppn
2. 然后再调用系统调用X,把这个ppn加入共享页\[id\]索引的位置
3. 被动方再调用系统调用X,然后输入想共享的ppn的索引，
4. 然后被动方不用申请物理页面，MapArea -> map_one -> ...

新的改变：

每个共享页面都需要走系统调用，然后index放的共享页索引

页面 = data_frame:修改（frametrace） + PageTableEntry:访问属性(页表里面) 

困难：如何实现只有一个共享页的时候是可写的，忽然加上一个共享，就变不可写（如何统计共享页是被那几个进程共享）



今天扫描了Linux共享机制，有了些许灵感，本来不打算写的，考研呢！但是这个cow机制成了我的心结，想把他消灭了！

- 默认第一个页面刚开始不共享
- 每个页面共享前，首先正常申请物理页，为了获取目标虚拟地址，然后共享页进行页替换就行
- 每个物理页在进行进程的共享的时候，把两者的物理页访问参数都改成只读
- 每个物理页在进行访问的时候，只是自己复制个物理页，然后自己创建个页进行复制，其他未涉及进行等到他修改的时候再改
- 所以上述过程不需要在某个页面被写时，统一改访问参数

这个代码到这里完成度已经很高了，不打算继续实现了！就是目前最后还有的问题是，在写只可读的页面的异常处理，与页面赋值的问题了，其他页面也是延迟更改访问权限。页面应用为0会自动释放页面。可能遇到的难题，重新分配一个物理页（有接口），然后重新绑定！，中间涉及一个页面复制的问题，他复制谁的（要么实现一个数组，储存每个共享页的分配进程，要么在更改之前，先使用临时页面储存复制前的数据），最后一个共享页不需要重新申请，直接使用后接口更改页面属性，然后踢出共享范畴。
...
- frametracker在new的时候，完成页面的清0
- frametracker在drop之后会进入回收器，所以在共享的时候，我们要判断他是否可以被回收
- 然后每个进程都有一个自己的页表项储存自己的访问属性

### sys_get_time（想完成）

引入虚存机制后，原来内核的 sys_get_time 函数实现就无效了。请你重写这个函数，恢复其正常功能。

### mmap 和 munmap 匿名映射（想完成）
